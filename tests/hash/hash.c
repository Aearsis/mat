...
/* Testing hash tables. The test:
 *	1) registers a hash table
 *	2) binds the MAT block to ingress
 *	3) sets a default chain
 *	4) inserts a matching flow
 *
 * After each step, a packet is delivered. Throughout the test, the subsystem
 * should:
	...
	struct mat_msg_arg flow_args [] = {
		{
			...
			.field.field = MAT_FIELD_ETHERNET_MAC_SRC,
			.field.value = 0x10203040506,
		},
		{
			.type = MAT_FLOW_ARG_ACTION,
			.action.op = MAT_ACT_PRINT,
